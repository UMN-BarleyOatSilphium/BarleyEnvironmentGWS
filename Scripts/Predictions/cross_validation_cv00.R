...
cv00_train_test <- cv_data %>%
  ...
  do({
    ...
    map(trait_env, ~{
      ...
      test_id <- subset(df, environment == env, c(line_name, id))
      train_id <- subset(df, environment != env, c(line_name, id))
      ...
      replicate(n = nCV, expr = crossv_kfold(data = cv_tp_df, k = k), simplify = FALSE) %>% 
        ...
        mutate(train = map(train, ~left_join(., train_id, by = "line_name")["id"]),
               test = map(test, ~left_join(., test_id, by = "line_name")["id"]))
    }) %>%
