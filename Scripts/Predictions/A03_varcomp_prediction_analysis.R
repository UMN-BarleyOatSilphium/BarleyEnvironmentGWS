...
file_list <- list.files(result_dir, pattern = "varcomp_predictions.RData$", full.names = TRUE)
...
loeo_interval_varcomp_predictions_out <- loeo_varcomp_interval_predictions_out %>%
  unnest(predictions, names_repair = tidyr_legacy) %>%
  select(model, feature_selection, out) %>%
  unnest(out)
...
lolo_varcomp_predictions_out <- location_varcomp_predictions_out %>%
  filter(type == "lolo") %>%
  unnest(predictions, names_repair = tidyr_legacy) %>%
  select(model, feature_selection, out) %>%
  unnest(out)

loc_external_varcomp_predictions_out <- location_varcomp_predictions_out %>%
...
rm(location_varcomp_predictions_out)
object_list <- ls(pattern = "varcomp_predictions_out")
...
output_filename <- paste0("prediction_accuracy_compiled_varcomp_", format(Sys.Date(), "%Y%m%d"), ".RData")
