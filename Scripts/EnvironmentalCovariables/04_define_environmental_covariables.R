...
# Prepare environment-concurrent covariates by interval -------------------------------

## Create the ECs and select the relevant ones for modeling
concurrent_interval_covariates1 <- concurrent_interval_covariates %>%
  inner_join(., env_trials, by = "trial") %>% 
  select(-trial) %>%
  gather(covariate, value, -environment, -stage, -source) %>%
  unite(covariate, stage, covariate, sep = ".") %>%
...
# Add soil covariates
ec_select_interval <- full_join(concurrent_interval_covariates1, soil_covariates1, by = "environment")


## Summarize min/max/var for each covariate
ec_select_interval_summ <- ec_select_interval %>%
  gather(covariate, value, -environment, -source) %>%
  group_by(covariate, source) %>%
  summarize_at(vars(value), list(min = min, max = max, sd = sd, cv = cv), 
               na.rm = T) %>%
...
## Remove covariates with low CV
ec_select_interval1 <- ec_select_interval_summ %>% 
  filter(!is.na(cv)) %>%
  select(source, covariate) %>%
  inner_join(., gather(ec_select_interval, covariate, value, -environment, -source)) %>%
  spread(covariate, value)


### Covariate diagnostics ###

ec_select_interval2 <- ec_select_interval1

## Test for normality using ks test
ec_tomodel_normality <- ec_select_interval2 %>%
  gather(covariate, value, -environment, -source) %>%
  group_by(source, covariate) %>%
  do(ks_test = ks.test(x = .$value, y = "pnorm", mean = mean(.$value, na.rm = T), sd = sd(.$value, na.rm = T))) %>%
  ungroup() %>%
...
# Impute with the mean
ec_select_interval3 <- ec_select_interval2 %>%
  gather(covariate, value, -environment, -source) %>%
  inner_join(., ec_to_keep) %>%
  spread(covariate, value) %>%
  split(.$source) %>%
...
## Calculate pairwise correlations
ec_tidy_cor <- ec_select_interval3 %>%
  split(.$source) %>%
  imap_dfr(~select(., -environment, -source) %>%
             cor() %>%
             as.dist() %>%
...
ec_select_interval4 <- ec_select_interval3 # %>% select(-subsoil_teb, -subsoil_ref_bulk_density)



## Prepare the covariates for modeling
## Center, but do not scale. Save the mean for later
ec_tomodel_interval_temp <- ec_select_interval4 %>%
  split(.$source) %>%
  map(~mutate_at(., vars(-environment, -source), scale, scale = FALSE, center = TRUE))

ec_tomodel_interval_centers <- ec_tomodel_interval_temp %>%
  map(~summarize_at(., vars(-environment, -source), ~attr(., "scaled:center")) %>%
        gather(covariate, center) )

## Convert scaled to numeric
ec_tomodel_interval_centered <- ec_tomodel_interval_temp %>%
  map(~mutate_at(., vars(-environment, -source), as.numeric))

## Center and scale
ec_tomodel_interval_scaled <- ec_select_interval4 %>%
  split(.$source) %>%
  map(~mutate_at(., vars(-environment, -source), ~as.numeric(scale(., scale = TRUE, center = TRUE))))
...
year_sliding_window <- c(window5 = 5, window10 = 10, window15 = 15) %>%
  map(~slider::slide(.x = all_historical_years, ~ ., .before = floor(.x / 2), .after = floor(.x / 2), .step = 1, .complete = TRUE)) %>%
...
# Prepare historical covariates summarized by interval -------------------------------------------


## Create the ECs and select the relevant ones for modeling
historical_location_interval_covariates1 <- historical_location_interval_covariates %>%
  ## Separate trial into location and year
  mutate(year = as.numeric(str_extract(trial, "[0-9]{4}")),
         location = str_remove(trial, "_[0-9]{4}")) %>%
  ## Replace Ithaca1 and Ithaca2 with Ithaca
...
soil_covariates1 <- soil_covariates %>%
  ...
  inner_join(., loc_trials, by = "location") %>%
  gather(covariate, value, -trial, -location) %>%
  left_join(distinct(historical_location_interval_covariates1, year, trial, location), .) %>%
  # Cross with sources of weather data
  crossing(., source = unique(historical_location_interval_covariates1$source))

## Combine
hist_ec_interval_select <- bind_rows(historical_location_interval_covariates1, soil_covariates1) %>%
  # Select only those covariates used for individual environments
  filter(covariate %in% names(ec_select_interval4))
...
# Create the vector of years
all_historical_years <- seq(max(hist_ec_interval_select$year) - 30 + 1, max(hist_ec_interval_select$year))
...
## Use the time frames to summarize covariates
ec_select_interval_timeframe <- tibble(
  time_frame = paste0("time_frame", map_chr(year_time_frame, ~paste(length(.), min(.), max(.), sep = "_"))),
  years = year_time_frame) %>%
  # Subset the historical EC
  mutate(ec_data = map(years, ~subset(hist_ec_interval_select, year %in% .x)))

# Summarize - calculate the mean covariate for each location across years
ec_select_interval_timeframe_summary <- ec_select_interval_timeframe %>%
  unnest(ec_data) %>%
  group_by(source, time_frame, covariate, location) %>%
  summarize(value = mean(value)) %>%
  ungroup()
...
## Use sliding windows to summarize covariates
ec_select_interval_window <- tibble(
  time_frame = unlist(imap(year_sliding_window, function(l, name) map_chr(l, ~paste0(name, "_", min(.x), "_", max(.x))))),
  years = unlist(year_sliding_window, recursive = FALSE)) %>%
  # Subset the historical EC
  mutate(ec_data = map(years, ~subset(hist_ec_interval_select, year %in% .x)))
...
# Summarize
ec_select_interval_window_summary <- ec_select_interval_window %>%
  unnest(ec_data) %>%
  group_by(source, time_frame, covariate, location) %>%
  summarize(value = mean(value)) %>%
  ungroup()
...
## Spread the data
ec_select_interval_timeframe_summary_wide <- spread(ec_select_interval_timeframe_summary, covariate, value)
ec_select_interval_window_summary_wide <- spread(ec_select_interval_window_summary, covariate, value)
...
# Number of distinct covariates (number of independent tests)
nTests <- n_distinct(ec_select_interval_timeframe_summary$covariate)

## Test for normality using ks test
ec_timeframe_normality <- ec_select_interval_timeframe_summary %>%
  group_by(source, covariate, time_frame) %>%
  do(ks_test = ks.test(x = .$value, y = "pnorm", mean = mean(.$value, na.rm = T), sd = sd(.$value, na.rm = T))) %>%
  ungroup() %>%
  mutate(p_value = map_dbl(ks_test, "p.value"),
...
# Repeat for window
## Test for normality using ks test
ec_window_normality <- ec_select_interval_window_summary %>%
  group_by(source, covariate, time_frame) %>%
  do(ks_test = ks.test(x = .$value, y = "pnorm", mean = mean(.$value, na.rm = T), sd = sd(.$value, na.rm = T))) %>%
  ungroup() %>%
  mutate(p_value = map_dbl(ks_test, "p.value"),
...
# Impute with the mean
ec_select_interval_timeframe_summary_wide1 <- ec_select_interval_timeframe_summary_wide %>%
  split(.$source) %>%
  # Inpute using the mean
  map_df(., ~mutate_at(., vars(-time_frame, -source, -location), impute))


ec_select_interval_window_summary_wide1 <- ec_select_interval_window_summary_wide %>%
  split(.$source) %>%
  # Inpute using the mean
  map_df(., ~mutate_at(., vars(-time_frame, -source, -location), impute))
...
## Calculate pairwise correlations ##
# Timeframe
ec_timeframe_tidy_cor <- ec_select_interval_timeframe_summary_wide1 %>%
  select(-location) %>%
  split(list(.$source, .$time_frame)) %>%
  imap_dfr(~cor(.x[,-1:-2]) %>% as.dist() %>% tidy() %>% 
         rename_all(~c("covariate1", "covariate2", "correlation")) %>%
...
# Window
ec_window_tidy_cor <- ec_select_interval_window_summary_wide1 %>%
  select(-location) %>%
  split(list(.$source, .$time_frame)) %>%
  imap_dfr(~cor(.x[,-1:-2]) %>% as.dist() %>% tidy() %>% 
             rename_all(~c("covariate1", "covariate2", "correlation")) %>%
...
## 
## Center, but do not scale. Save the mean for later
historical_ec_tomodel_timeframe_interval_temp <- ec_select_interval_timeframe_summary_wide1 %>%
  split(list(.$source, .$time_frame)) %>%
  map(~mutate_at(.x, vars(-source, -time_frame, -location), scale, scale = FALSE, center = TRUE))

historical_ec_tomodel_timeframe_interval_centers <- historical_ec_tomodel_timeframe_interval_temp %>%
  map(~summarize_at(.x, vars(-source, -time_frame, -location), ~attr(., "scaled:center")) %>%
        gather(covariate, center) )

## Convert scaled to numeric
historical_ec_tomodel_timeframe_interval_centered <- historical_ec_tomodel_timeframe_interval_temp %>%
  map(~mutate_at(.x, vars(-time_frame, -location, -source), as.numeric) )

## Center and scale
historical_ec_tomodel_timeframe_interval_scaled <- ec_select_interval_timeframe_summary_wide1 %>%
  split(list(.$source, .$time_frame)) %>%
  map(~mutate_at(.x, vars(-time_frame, -location, -source), scale, scale = TRUE, center = TRUE) %>%
        mutate_at(vars(-time_frame, -location, -source), as.numeric) )
...
## 
## Center, but do not scale. Save the mean for later
historical_ec_tomodel_window_interval_temp <- ec_select_interval_window_summary_wide1 %>%
  split(list(.$source, .$time_frame)) %>%
  map(~mutate_at(.x, vars(-time_frame, -location, -source), scale, scale = FALSE, center = TRUE))

historical_ec_tomodel_window_interval_centers <- historical_ec_tomodel_window_interval_temp %>%
  map(~summarize_at(.x, vars(-time_frame, -location, -source), ~attr(., "scaled:center")) %>%
        gather(covariate, center) )

## Convert scaled to numeric
historical_ec_tomodel_window_interval_centered <- historical_ec_tomodel_window_interval_temp %>%
  map(~mutate_at(.x, vars(-time_frame, -location, -source), as.numeric) )

## Center and scale
historical_ec_tomodel_window_interval_scaled <- ec_select_interval_window_summary_wide1 %>%
  split(list(.$source, .$time_frame)) %>%
  map(~mutate_at(.x, vars(-time_frame, -location, -source), scale, scale = TRUE, center = TRUE) %>%
        mutate_at(vars(-time_frame, -location, -source), as.numeric) )
...
save("ec_tomodel_centered", "ec_tomodel_scaled", "ec_tomodel_centers", 
     "ec_tomodel_interval_centered", "ec_tomodel_interval_scaled", "ec_tomodel_interval_centers",
     "historical_ec_tomodel_timeframe_centered", "historical_ec_tomodel_timeframe_scaled", "historical_ec_tomodel_timeframe_centers",
     "historical_ec_tomodel_timeframe_interval_centered", "historical_ec_tomodel_timeframe_scaled", "historical_ec_tomodel_timeframe_centers",
     "historical_ec_tomodel_window_centered", "historical_ec_tomodel_window_scaled", "historical_ec_tomodel_window_centers",
     "historical_ec_tomodel_window_interval_centered", "historical_ec_tomodel_window_interval_scaled", "historical_ec_tomodel_window_interval_centers",
