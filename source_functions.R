...
model1 <- function(train, test, Kg) {
  ...
  # Fit
  fit <- mmer(fixed = value ~ 1 + environment, random = ~ vs(line_name, Gu = Kg), 
              data = mf, verbose = FALSE, date.warning = FALSE)
...
model2 <- function(train, test, Kg) {
  ...
  Kge <- kronecker(Kg, E, make.dimnames = TRUE)
  # colnames(Zge) <- colnames(Kge)
  
  # Fit
  fit <- mmer(fixed = value ~ 1 + environment, random = ~ vs(line_name, Gu = Kg) + vs(line_name:environment, Gu = Kge), 
              data = mf, verbose = FALSE, date.warning = FALSE)
  ...
  ge <- randef(fit)[[2]] %>% 
    ...
    separate(col = term, c("line_name", "environment"), sep = ":") %>%
    rename(ge = value)
